# [Nivel IV · Libro 7] Private Equity y Venture Capital

> Hasta aquí aprendiste a **comprar** (M&A), **financiar** (deuda) y **colaborar** (JV). Ahora subes al
> lugar donde todas esas herramientas se usan con la **máxima intensidad y sofisticación**: el mundo de
> los **inversionistas profesionales**. El **private equity (PE)** y el **venture capital (VC)** son los
> fondos que invierten capital de terceros en empresas —maduras (PE) o emergentes (VC)— para hacerlas
> crecer y **venderlas con ganancia** años después. Para el abogado corporativo, son los **clientes más
> sofisticados y exigentes**, y dominar su lógica —cómo se estructura un fondo, cómo invierte, cómo se
> protege y cómo sale— es entrar a la élite de la práctica. Este libro reúne y eleva todo lo aprendido:
> el LBO (Libros 3 y 5), el SHA con sus preferencias y salidas (Libro 4) y la coinversión (Libro 6),
> vistos desde el lado del **fondo**.

**Etiquetas:** corporate law · private equity · venture capital · fondos · LBO · term sheet de inversión · cap table · exit
**Prerrequisitos:** todo el Nivel IV previo, en especial [Libro 3 · Estructuras](./03-Estructuras-Fusiones-Escisiones-Adquisiciones.md), [Libro 4 · Documentación (SHA)](./04-Documentacion-Term-Sheet-SPA-APA-SHA.md), [Libro 5 · Financiamiento (LBO)](./05-Derecho-Bancario-y-Financiamiento-Corporativo.md) y [Libro 6 · JV](./06-Contratos-Estrategicos-y-Joint-Ventures.md) + [Gobierno Corporativo](../Nivel-III-Derecho-Mercantil/07-Gobierno-Corporativo.md) y [Mercado de Valores (LMV)](../Nivel-III-Derecho-Mercantil/05-Mercado-de-Valores-LMV.md).
**Estándar:** V3 (43 secciones + secciones transversales).
**⏱ Tiempo estimado de dominio:** 6–8 semanas (≈ 70–90 h de estudio activo). Lectura de
reconocimiento: 12–15 h. *Estúdialo en bloques: la lógica del fondo (GP/LP) → private equity y el LBO →
venture capital y las rondas → el term sheet de inversión (preferencias, antidilución, vesting) → el cap
table y las salidas (exit).*

> **Aviso de método.** Tratado, no repaso. Aprenderás a pensar como el **inversionista profesional**:
> cómo levanta y estructura un fondo, cómo invierte (PE vía LBO; VC vía rondas), cómo se **protege**
> (preferencias, vetos, antidilución, vesting) y cómo **sale** (venta, IPO) —porque el retorno se realiza
> en la salida—. Cada figura reúne lo aprendido en todo el Nivel IV.

---

### Nivel de importancia profesional (vista rápida)
```
Litigio              ★★☆☆☆
Corporate            ★★★★★
M&A                  ★★★★★
Mercado de Valores   ★★★★☆
Venture Capital      ★★★★★
Private Equity       ★★★★★
Gobierno Corporativo ★★★★★
Compliance           ★★★★☆
Derecho Bancario     ★★★★☆
Empresa Familiar     ★★★☆☆
```
*Los fondos de PE/VC son los clientes más sofisticados y recurrentes del abogado corporativo de élite, y
generan un flujo constante de operaciones (inversión, gestión, salida). Dominar su lógica es entrar al
corazón de la práctica de operaciones complejas.*

## 1. Introducción

El **private equity (PE)** y el **venture capital (VC)** son dos formas de **inversión de capital privado**:
fondos que reúnen dinero de inversionistas y lo invierten en **empresas que no cotizan en bolsa** (de ahí
"privado"), con un objetivo claro: **hacerlas crecer y venderlas con ganancia** en un horizonte de años.
La diferencia entre ambos está, sobre todo, en el **tipo de empresa** y la **etapa**:
- El **private equity** invierte en empresas **maduras y consolidadas** (con flujo, historia, activos),
  típicamente **comprando el control** (a menudo con deuda —el **LBO**—) para profesionalizarlas,
  hacerlas crecer y revenderlas.
- El **venture capital** invierte en empresas **jóvenes y de alto crecimiento** (*startups*), tomando
  participaciones **minoritarias** en sucesivas **rondas** de financiación, apostando a que algunas se
  vuelvan muy valiosas (aunque muchas fracasen).

La idea rectora del libro: **el inversionista profesional compra para vender, y todo lo que hace está
orientado a la salida (el *exit*).** A diferencia de un dueño que quiere conservar su empresa, el fondo
tiene un **horizonte definido** (típicamente 5-10 años) y una **misión única**: comprar bien, crear valor,
y **salir** (vender o sacar a bolsa) con la mayor ganancia. Esta lógica —*entrar pensando en cómo salir*—
explica **cada** decisión y **cada** cláusula que el fondo negocia: por qué pide preferencia de liquidación
(cobrar primero al salir), por qué quiere drag along (poder vender el 100%), por qué exige vetos (proteger
su inversión hasta la salida), por qué estructura un LBO (amplificar el retorno con deuda).

Para el abogado corporativo, los fondos de PE/VC son los **clientes más valiosos y exigentes**: hacen
operaciones constantemente (invierten, gestionan, salen), son sofisticadísimos, y su trabajo reúne **todo**
lo que has aprendido en el Nivel IV. Una inversión de PE es un M&A (compra) + estructura (NewCo) +
financiamiento (LBO) + SHA (gobierno y salida) + due diligence, todo combinado. Una inversión de VC es una
suscripción + un SHA especializadísimo (preferencias, antidilución, vesting) + una apuesta sobre el
crecimiento. Este libro te enseña a verlo **desde el lado del fondo** —y, con ello, a integrar toda tu
formación en la lógica del inversionista que mueve el capital del mundo—.

## 2. Objetivos del libro

Al terminar, el lector será capaz de:

- **Comprender la estructura de un fondo** de PE/VC: la relación **GP/LP** (*General Partner* / *Limited
  Partners*), el *fund*, las comisiones (*management fee* y ***carried interest***), el ciclo de vida del
  fondo (*fundraising*, inversión, gestión, *exit*) y los vehículos (en México y *offshore*).
- **Dominar la lógica del private equity**: la **tesis de inversión** (comprar, mejorar, vender), el
  **LBO** (apalancamiento para amplificar el retorno —reuniendo los Libros 3 y 5—), la creación de valor
  operativa y financiera, y los *exits* (venta estratégica, *secondary*, IPO).
- **Dominar la lógica del venture capital**: las **etapas** (semilla, Serie A, B, C...), la **valuación**
  (*pre-money* / *post-money*), la **dilución**, el *power law* (pocos éxitos pagan muchos fracasos) y el
  papel del VC en el ecosistema de *startups*.
- **Diseccionar el term sheet de inversión** de VC/PE: la **valuación**, la **preferencia de liquidación**
  (1x, participating/non-participating), la **antidilución** (full ratchet / weighted average), los
  **derechos de gobierno** (board, vetos/*protective provisions*), los **derechos de información**, el
  **pro-rata**, y el **vesting** de fundadores (*good/bad leaver*).
- **Leer y construir un *cap table*** (tabla de capitalización): cómo se reparte la propiedad, cómo cambia
  con cada ronda, y el efecto de las opciones (*ESOP*) y los instrumentos convertibles (**SAFE**, nota
  convertible).
- **Dominar las salidas (*exit*)**: la venta estratégica (*trade sale*), la venta secundaria, el *IPO*, y
  los mecanismos del SHA que las habilitan (drag along, registration rights, redemption).
- **Conectar** el PE/VC con todo el Nivel IV: ver la inversión del fondo como la **síntesis** de M&A,
  estructura, financiamiento, SHA y due diligence, vista desde el inversionista.

## 3. Importancia profesional

Para el abogado corporativo de élite, los fondos de **PE y VC son los clientes más codiciados**: hacen
operaciones **constantemente** (un fondo invierte en varias empresas, las gestiona durante años, y sale de
ellas), son **sofisticadísimos** (saben exactamente lo que quieren), y pagan por el **mejor trabajo**.
Atender a un fondo de PE/VC significa estar en el **centro del deal flow** del mercado: ver y estructurar
las operaciones más interesantes, una tras otra.

Para **tu perfil específico** —estructuración de operaciones complejas, M&A, gobierno corporativo,
financiamiento—, el PE/VC es el campo donde **todo converge**. Una operación de PE típica reúne: un **M&A**
(la compra), una **estructura** (NewCo, holding —Libro 3—), un **financiamiento/LBO** (Libro 5), un **SHA**
(gobierno y salida —Libro 4—), una **due diligence** (Libro 2), y a veces una **JV** o coinversión (Libro
6). Una operación de VC reúne una suscripción y un SHA especializadísimo. Dominar el PE/VC es, en la
práctica, **dominar la aplicación integrada** de todo lo que has estudiado —y hacerlo para los clientes
más exigentes—.

Hay además una razón de **trayectoria profesional**: muchos de los mejores abogados corporativos terminan
**trabajando con o dentro de fondos** (como asesores externos recurrentes, como *general counsel* del
fondo, o incluso pasando al lado de la inversión). El PE/VC es también un motor de la **economía** (financia
la innovación —VC— y la consolidación y profesionalización de empresas —PE—), por lo que entenderlo te
conecta con cómo se mueve el capital y se crea valor en el mundo real. Para quien aspira a la élite del
derecho corporativo, el PE/VC **no es opcional**: es el destino natural.

## 4. Historia y origen

El **private equity** moderno nace en Estados Unidos a mediados del siglo XX, pero explota en los **años
1980** con la era de los **leveraged buyouts (LBO)**: firmas como KKR (Kohlberg Kravis Roberts) demostraron
que se podían comprar empresas enormes usando **mucha deuda** y poco capital propio, reestructurarlas y
revenderlas con ganancias extraordinarias. El icónico LBO de **RJR Nabisco** (1988) —narrado en *Barbarians
at the Gate*— marcó la cúspide (y los excesos) de esa era. Desde entonces, el PE se institucionalizó: hoy
mueve **billones** de dólares globalmente, con firmas como Blackstone, KKR, Carlyle y Apollo.

El **venture capital** tiene raíces igualmente estadounidenses: la inversión en empresas tecnológicas
emergentes que floreció con **Silicon Valley** a partir de los años 1960-70 (la financiación de
semiconductores, luego computación, internet, biotecnología). Firmas como Sequoia, Kleiner Perkins y
Andreessen Horowitz financiaron a las empresas que definieron la era digital (Apple, Google, Amazon y
muchas más nacieron con capital de riesgo). El VC desarrolló su propia lógica: apostar a **muchas**
*startups* sabiendo que la mayoría fracasará, pero que **unas pocas** generarán retornos que pagan todo el
resto (el *power law*).

En **México y América Latina**, el PE/VC se desarrolló más tarde (sobre todo desde los años 2000, y con
fuerza en la última década). El **PE** mexicano creció con la consolidación de sectores y la
profesionalización de empresas familiares; surgieron vehículos locales como los **CKDs** (Certificados de
Capital de Desarrollo) y **CERPIs** que permiten a las **afores** invertir en capital privado. El **VC**
latinoamericano vivió un *boom* notable, con el surgimiento de *unicornios* regionales (empresas valuadas
en más de mil millones de dólares) y la llegada de fondos globales. La figura societaria de la **S.A.P.I.**
(creada en la LMV) fue diseñada, en buena parte, **para** facilitar estas inversiones (validando las
cláusulas de los inversionistas). Hoy el PE/VC es una práctica jurídica vibrante y creciente en México.

## 5. Evolución histórica

La evolución del capital privado puede leerse en tres movimientos:

**1) Del LBO oportunista a la creación de valor operativa.** El PE de los años 1980 era, en buena medida,
**ingeniería financiera**: comprar con deuda, recortar, revender (a veces "descuartizando" empresas). El PE
maduro **cambió el énfasis**: hoy el valor se crea no solo con apalancamiento, sino **mejorando
operativamente** las empresas (profesionalizar la gestión, expandir, hacer adquisiciones complementarias
—*buy-and-build*—, optimizar). El abogado acompaña esa creación de valor (las adquisiciones *add-on*, la
gobernanza, la preparación para la venta).

**2) La sofisticación de los instrumentos del VC.** El VC desarrolló un **arsenal jurídico propio** y cada
vez más estandarizado: las **acciones preferentes** con sus preferencias de liquidación y antidilución, el
**vesting** de fundadores, los instrumentos de inversión rápida (**nota convertible**, y luego el **SAFE**
—*Simple Agreement for Future Equity*, creado por Y Combinator—), los *term sheets* estándar (los modelos
de NVCA en EE.UU.). Esta estandarización aceleró la inversión y creó un **lenguaje común** global del VC.

**3) La globalización y la institucionalización del capital privado.** El PE/VC pasó de ser una actividad
de nicho a una **clase de activo institucional**: los grandes inversionistas (fondos de pensiones,
aseguradoras, fondos soberanos, *endowments*) destinan porciones significativas de su portafolio al capital
privado. En México, los **CKDs/CERPIs** abrieron el capital privado a las **afores**. Y el VC se globalizó:
los fondos invierten cross-border, y ecosistemas como el latinoamericano atrajeron capital global. El
resultado: más capital, más operaciones, y una práctica jurídica cada vez más sofisticada y
estandarizada internacionalmente.

## 6. Contexto económico

El PE/VC es **economía financiera** en estado puro. Tres ideas la gobiernan:

- **El fondo como intermediario que busca retornos superiores.** Un fondo de PE/VC reúne capital de
  **inversionistas** (LPs) que buscan **retornos superiores** a los del mercado público (a cambio de menos
  liquidez y más riesgo). El fondo (gestionado por el GP) invierte ese capital en empresas privadas,
  buscando un retorno medido por el **IRR** (*tasa interna de retorno*) y el **múltiplo** (cuántas veces
  devuelve el capital). Su modelo de negocio: cobrar una **comisión de gestión** (*management fee*, ~2%
  anual) y, sobre todo, quedarse con una parte de las **ganancias** (*carried interest* o *carry*, ~20% de
  las utilidades por encima de un umbral —*hurdle rate*—). El célebre "**2 y 20**".
- **La creación de valor: cómo el fondo gana dinero.** El PE crea valor de tres formas: **(i)
  apalancamiento** (el LBO: usar deuda para amplificar el retorno del equity —Libro 5—); **(ii) mejora
  operativa** (crecer, profesionalizar, hacer add-ons, optimizar márgenes); y **(iii) *multiple
  expansion*** (vender a un múltiplo mayor que al que compró —comprar barato, vender caro—). El VC crea
  valor distinto: apuesta al **crecimiento exponencial** de *startups*, donde el valor viene del
  **potencial** (no del flujo actual).
- **El *power law* del venture capital.** La economía del VC es **asimétrica**: de cada 10 inversiones,
  varias fracasan (pérdida total), algunas devuelven el capital, y **una o dos** generan retornos enormes
  (10x, 50x, 100x) que **pagan todo el portafolio y más**. Esto explica la lógica del VC: no busca evitar
  fracasos (son inevitables), sino **no perderse el gran éxito**; por eso invierte en muchas, apuesta al
  **upside** ilimitado, y estructura sus protecciones (preferencias) para los escenarios intermedios. El PE,
  en cambio, busca retornos más **consistentes** (menos fracasos, menos *home runs*).

## 7. Contexto político y regulatorio

El PE/VC opera bajo un marco regulatorio que el abogado debe dominar:

- **Regulación de los fondos y los vehículos.** La estructura del fondo está regulada: en México, los
  **CKDs** y **CERPIs** (vehículos bursátiles que permiten a las **afores** y otros institucionales
  invertir en capital privado) están bajo la **LMV** y la supervisión de la **CNBV**; los fondos
  *offshore* (típicamente en jurisdicciones como Delaware, Cayman) tienen su propia regulación. La relación
  GP/LP se rige por el contrato del fondo (*Limited Partnership Agreement* / *LPA*).
- **La S.A.P.I. y el marco societario (LMV/LGSM).** La **S.A.P.I.** fue diseñada para **dar validez
  societaria** a las cláusulas que exigen los inversionistas (preferencias, vetos, drag/tag, opciones,
  vesting), siendo el vehículo preferido para las empresas que reciben inversión de PE/VC en México.
- **Competencia (COFECE).** Las adquisiciones de control del PE (LBO) pueden ser **concentraciones
  notificables**; los fondos que tienen varias empresas de un mismo sector deben cuidar la competencia.
- **Mercado de valores (LMV) para las salidas vía IPO.** Si el *exit* es una **salida a bolsa**, entra el
  régimen de emisoras (prospecto, revelación, OPAs).
- **Fiscal.** La estructura de los fondos y de las inversiones es **muy sensible fiscalmente** (cómo se
  gravan el *carry*, las ganancias de capital, los vehículos *offshore*); el diseño fiscal es central y se
  hace con especialistas.
- **Regulación de *startups* y *fintech*.** El VC invierte a menudo en sectores regulados emergentes
  (fintech —Ley Fintech en México—, salud digital, etc.), donde la regulación condiciona el negocio.

## 8. Contexto social

El PE/VC tiene una dimensión social y un debate público intensos. Por el lado **positivo**: el **VC
financia la innovación** —las empresas que transforman la economía (tecnología, salud, energía limpia)
suelen nacer con capital de riesgo—, generando empleo, productividad y avance tecnológico; y el **PE
profesionaliza y rescata empresas** (moderniza compañías ineficientes, salva empresas en problemas,
permite la sucesión de empresas familiares cuyos dueños quieren retirarse). El capital privado es un motor
de **dinamismo económico**.

Por el lado **crítico**: el PE ha sido cuestionado por los **excesos del apalancamiento** (cargar de deuda
a empresas, a veces llevándolas a la quiebra mientras el fondo ya extrajo su ganancia), por los **recortes**
(reducir empleos para mejorar márgenes y vender), y por la **extracción de valor** a corto plazo. El debate
sobre si el PE "crea" o "extrae" valor es genuino y matizado (hay de todo: fondos que construyen y fondos
que descuartizan). El abogado serio entiende esta tensión y opera con **criterio ético**: estructurar
operaciones que creen valor real, no solo que extraigan.

Para el **fundador** de una *startup*, el VC tiene una dimensión humana intensa: recibir capital de riesgo
significa **ceder parte del control y de la propiedad** a cambio de recursos para crecer, y aceptar la
presión de un inversionista que **quiere salir** con ganancia. El abogado del fundador debe equilibrar la
necesidad de capital con la protección del fundador (que no lo diluyan o expulsen injustamente —de ahí la
importancia del *vesting*, el *good/bad leaver*, y de no ceder vetos excesivos—). Hay un **conflicto
estructural de intereses** entre fundador (quiere crecer y conservar control) e inversionista (quiere
retorno y salida), y el abogado opera en esa tensión.

## 9. Contexto empresarial

Desde la óptica empresarial, el PE/VC cumple funciones distintas según la etapa de la empresa:

- **Para la *startup* (VC):** el capital de riesgo es, a menudo, la **única forma** de financiar el
  crecimiento de una empresa joven que **no tiene flujo ni activos** para obtener deuda bancaria. El VC
  aporta **capital + acompañamiento** (mentoría, red de contactos, credibilidad). A cambio, el fundador
  cede equity y acepta la lógica de crecer rápido para una eventual salida.
- **Para la empresa madura (PE):** el private equity ofrece **capital + profesionalización + una salida
  para los dueños actuales**. Sirve para: financiar el crecimiento (orgánico o por adquisiciones),
  **profesionalizar** una empresa familiar, permitir la **sucesión** (los dueños venden y se retiran),
  **rescatar** una empresa en problemas (*turnaround*), o tomar una empresa pública y volverla privada
  (*public-to-private*).
- **Para los inversionistas (LPs):** el PE/VC es una **clase de activo** que ofrece (en teoría) retornos
  superiores a cambio de iliquidez y riesgo; las afores, aseguradoras y family offices invierten en
  fondos para diversificar y buscar rendimiento.

La decisión empresarial clave: **¿conviene recibir capital de un fondo?** Para una *startup*, el VC puede
ser el combustible del crecimiento, pero a costa de control y con presión de salida. Para una empresa
madura, el PE puede traer capital y profesionalización, pero significa ceder el control (o venderse) a un
socio cuyo objetivo es **revender en años**. El abogado ayuda a entender qué se gana y qué se cede, y a
**estructurar la entrada del fondo** protegiendo los intereses de su cliente (sea el fondo o la empresa/
fundador). Entender la **lógica del fondo** —que entra pensando en cómo salir— es esencial para negociar
bien de cualquiera de los dos lados de la mesa.

## 10. Definiciones doctrinales

La doctrina financiera y jurídica define los conceptos centrales así:

- **Private Equity (PE) / Capital privado:** inversión de capital en empresas **maduras que no cotizan**,
  típicamente **adquiriendo el control** (a menudo con apalancamiento —LBO—), para crear valor y revender
  en un horizonte de años. La doctrina lo define como una **clase de activo** de inversión alternativa
  orientada al **control y la transformación** de empresas establecidas.
- **Venture Capital (VC) / Capital de riesgo:** inversión en empresas **jóvenes de alto crecimiento**
  (*startups*), mediante participaciones **minoritarias** en sucesivas **rondas**, apostando al crecimiento
  exponencial de unas pocas que compensan los fracasos de muchas (*power law*).
- **Fondo (*fund*):** vehículo de inversión que **reúne capital** de varios inversionistas para invertirlo
  según una estrategia, con un horizonte definido. Jurídicamente, suele estructurarse como una
  **sociedad/asociación** (en EE.UU., *Limited Partnership*; en México, vía **CKD/CERPI** o vehículos
  *offshore*).
- **GP (*General Partner*) / Gestor:** quien **gestiona** el fondo, toma las decisiones de inversión y
  responde por ellas; cobra comisión de gestión y *carry*. Es el "**administrador**" del fondo.
- **LP (*Limited Partner*) / Inversionista:** quien **aporta el capital** al fondo (afores, aseguradoras,
  fondos de pensiones, family offices, soberanos), con responsabilidad limitada a su aportación y un papel
  **pasivo** (no gestiona).
- **Carried interest (*carry*):** la **participación en las ganancias** que cobra el GP (típicamente ~20%
  de las utilidades por encima de un umbral —*hurdle*—). Es el principal incentivo del gestor.
- **LBO (*Leveraged Buyout*):** adquisición de una empresa financiada en gran parte con **deuda**, repagada
  con el flujo de la propia empresa adquirida (visto en Libros 3 y 5).
- **Term Sheet de inversión:** documento que fija los **términos** de la inversión del fondo (valuación,
  preferencias, gobierno), mayormente **no vinculante** salvo cláusulas de proceso (eco del Libro 4).
- **Preferencia de liquidación (*liquidation preference*):** derecho del inversionista a **cobrar primero**
  (un múltiplo de su inversión) en una venta o liquidación, antes que los ordinarios.
- **Cap table (tabla de capitalización):** el registro de **quién posee qué** porcentaje del capital de la
  empresa, y cómo cambia con cada ronda, opción e instrumento convertible.
- **Exit (salida):** la operación por la que el fondo **realiza su inversión** —vendiéndola (a un
  estratégico o a otro fondo) o sacándola a bolsa (IPO)—. Es el momento en que se materializa el retorno.

## 11. Definiciones legales y marco normativo

El PE/VC se asienta en un entramado de derecho societario, bursátil y contractual:

- **Estructura del fondo:** la relación **GP/LP** se rige por el contrato del fondo (***LPA — Limited
  Partnership Agreement***, en vehículos extranjeros) o por las reglas del vehículo mexicano. En México,
  los **CKDs** (Certificados de Capital de Desarrollo) y **CERPIs** (Certificados de Proyectos de
  Inversión) son **vehículos bursátiles** (regulados por la **LMV** y la CNBV) que permiten a las **afores**
  y otros institucionales invertir en capital privado/proyectos. Muchos fondos usan también vehículos
  *offshore* (Delaware LP, Cayman) por eficiencia.
- **La S.A.P.I. (LMV): el vehículo de la empresa invertida.** La **Sociedad Anónima Promotora de
  Inversión** fue creada (LMV) precisamente para **dar validez societaria** a las cláusulas que exigen los
  inversionistas de PE/VC: **restricciones a la transmisión** (lock-up, ROFR, tag/drag), **derechos de
  veto**, **acciones con derechos especiales** (preferencias), **opciones** (put/call) y **convenios entre
  accionistas**. Es el vehículo estándar de la empresa que recibe inversión.
- **Acciones preferentes y series (LGSM/LMV).** El derecho societario permite **distintas series de
  acciones** con derechos diferenciados (las acciones **preferentes** del inversionista, con preferencia de
  liquidación y derechos de gobierno, frente a las **ordinarias** de los fundadores).
- **Instrumentos convertibles.** Las **notas convertibles** (deuda que se convierte en acciones en la
  siguiente ronda) y los **SAFE** (*Simple Agreement for Future Equity* —no es deuda ni equity hasta la
  conversión—) se estructuran sobre la base contractual (autonomía de la voluntad), adaptados al derecho
  mexicano.
- **Convenios entre accionistas (SHA — Libro 4).** El corazón documental de la inversión: validado por la
  S.A.P.I., rige gobierno, preferencias, antidilución, vesting y salidas.
- **Mercado de valores (LMV) para el IPO.** Si la salida es a bolsa, aplica el régimen de emisoras.
- **Competencia (LFCE), fiscal (LISR/CFF) e inversión extranjera.** Condicionan las adquisiciones (LBO), la
  estructura fiscal del fondo y del *carry*, y la participación extranjera.

> **Nota de método:** la estructura de fondos e inversiones es **intensamente fiscal y regulatoria**;
> verifica el marco vigente (LMV, reglas de CKD/CERPI, LISR) y estructura **con especialistas fiscales**
> antes de aplicar (Manifiesto XI.5).

## 12. Definición sencilla

Imagina dos tipos de inversionistas que ayudan a las empresas a crecer, para luego venderlas con ganancia:

**El Venture Capital (VC) es como un "buscador de estrellas".** Apuesta a **muchas bandas de garage**
(*startups*): le da dinero a 10 bandas jóvenes a cambio de un pedazo de cada una. Sabe que **7 fracasarán**,
2 quedarán bien, y **1 se volverá una superestrella** que vale tanto que **paga las otras 9 y deja enorme
ganancia**. No busca evitar fracasos (son inevitables); busca **no perderse la estrella**. Por eso invierte
en muchas, en **etapas** (primero poco dinero —semilla—, luego más si la banda despega —Serie A, B...—), y
se protege para los casos intermedios.

**El Private Equity (PE) es como un "renovador de casas".** Compra una **casa sólida pero descuidada** (una
empresa madura), normalmente con **un préstamo grande** (apalancamiento: pone poco de su dinero y mucho
prestado), la **renueva** (la profesionaliza, la hace crecer), y la **revende** más cara unos años después.
Su ganancia viene de **comprar bien, mejorar, y vender mejor** —amplificada por la deuda—.

Lo que ambos tienen en común, y que **explica todo lo que hacen**: **entran pensando en cómo salir**. No
quieren quedarse con la empresa para siempre; quieren **venderla con ganancia** en unos años. Por eso, cada
cláusula que negocian responde a una pregunta: *si esto va bien, ¿cómo me aseguro de cobrar mucho?; si va
regular, ¿cómo recupero al menos lo mío (preferencia de liquidación)?; si quiero vender todo, ¿cómo obligo
a los demás a vender conmigo (drag along)?; mientras tanto, ¿cómo protejo mi inversión (vetos)?*. El fondo
es un inversionista con **fecha de salida**, y eso lo explica todo.

## 13. Conceptos fundamentales

Fija este vocabulario; es el idioma del capital privado y reúne términos del SHA (Libro 4) y del LBO
(Libro 5):

**La estructura del fondo:**
- **GP / LP:** gestor (administra, cobra carry) / inversionista (aporta capital, pasivo).
- **Management fee / Carried interest:** comisión de gestión (~2%) / participación en ganancias (~20%) =
  "**2 y 20**".
- **Hurdle rate / Preferred return:** rendimiento mínimo que se da a los LPs antes de que el GP cobre carry.
- **Commitment / Drawdown / Capital call:** capital comprometido por el LP / llamadas de capital cuando el
  fondo invierte.
- **Vintage y ciclo de vida del fondo:** *fundraising* → periodo de inversión → gestión → *exit*/
  desinversión (típicamente 10 años).
- **CKD / CERPI:** vehículos mexicanos para que las afores inviertan en capital privado.

**Private Equity:**
- **Buyout / LBO:** compra de control, con apalancamiento (Libros 3 y 5).
- **Buy-and-build / add-ons:** crear valor adquiriendo empresas complementarias.
- **Multiple expansion:** vender a un múltiplo mayor que al que se compró.
- **Turnaround / distressed:** rescate de empresas en problemas.
- **Dry powder:** capital comprometido aún no invertido.

**Venture Capital:**
- **Etapas:** **pre-seed/semilla** → **Serie A** → **Serie B, C...** (cada ronda, más capital y mayor
  valuación si la empresa crece).
- **Pre-money / Post-money:** valuación **antes** / **después** de la nueva inversión (post = pre +
  inversión).
- **Dilución:** reducción del porcentaje de los socios existentes al emitir acciones nuevas.
- **Cap table:** tabla de capitalización (quién posee qué).
- **SAFE / Nota convertible:** instrumentos de inversión rápida que **convierten** en acciones en la
  siguiente ronda (con *descuento* y/o *cap* de valuación).
- **ESOP / pool de opciones:** acciones reservadas para **empleados** (incentivo); diluye a todos.
- **Unicornio:** *startup* valuada en más de mil millones de dólares.
- **Power law / home run:** la lógica de que pocos éxitos enormes pagan muchos fracasos.

**El term sheet de inversión y el SHA (protecciones del inversionista — reúne el Libro 4):**
- **Preferencia de liquidación:** cobrar primero (1x, 2x; *participating* / *non-participating*).
- **Antidilución:** protección si una ronda futura es a menor precio (*full ratchet* / *weighted average*).
- **Protective provisions / vetos:** decisiones que requieren consentimiento del inversionista.
- **Board seats / información:** asientos en el consejo y derechos de información.
- **Pro-rata / preemptive rights:** derecho a invertir en rondas futuras para no diluirse.
- **Vesting de fundadores:** las acciones del fundador se "ganan" con el tiempo (típico 4 años con 1 de
  *cliff*); **good leaver / bad leaver** (cómo se va).
- **Drag along / Tag along / ROFR:** salida (arrastrar / acompañar / preferencia) —del SHA—.
- **Registration rights:** derecho a exigir que se registren las acciones para venderlas en un IPO.

**Salidas (*exit*):**
- **Trade sale (venta estratégica):** vender a otra empresa del sector (M&A).
- **Secondary buyout:** vender a otro fondo de PE.
- **IPO (oferta pública inicial):** sacar la empresa a bolsa.
- **Redemption / recompra:** que la empresa o los socios recompren la participación del fondo.

Con este vocabulario, entramos al **desarrollo absoluto**.

## 14. Desarrollo absoluto

Esta es la sección central. La desarrollamos en cinco bloques: **(A)** la **estructura del fondo** (GP/LP,
comisiones, ciclo de vida); **(B)** el **private equity y el LBO** (la tesis, la creación de valor, el
apalancamiento); **(C)** el **venture capital y las rondas** (etapas, valuación, dilución, instrumentos);
**(D)** el **term sheet de inversión y el SHA** (preferencias, antidilución, vetos, vesting —el corazón
documental—); y **(E)** el **cap table y las salidas** (cómo se reparte la propiedad y cómo sale el fondo).
Estúdialos en orden: primero cómo se organiza el dinero (el fondo), luego cómo invierte (PE y VC), después
cómo se documenta y protege (term sheet/SHA), y al final cómo se realiza el retorno (cap table y exit).

### 14.A · La estructura del fondo (GP / LP, comisiones, ciclo de vida)

Antes de invertir en una sola empresa, hay que entender **cómo se organiza el dinero del inversionista
profesional**: el **fondo**. Un fondo de PE/VC no es una empresa que invierte su propio dinero; es un
**vehículo** que reúne capital de terceros para invertirlo según una estrategia.

**La relación GP / LP (la arquitectura básica).**
- Los **LPs (*Limited Partners* / inversionistas)** son quienes **ponen el capital**: afores, aseguradoras,
  fondos de pensiones, *endowments*, family offices, fondos soberanos. Son **pasivos** (no gestionan) y su
  responsabilidad se **limita** a su aportación. Buscan retornos superiores a cambio de iliquidez y riesgo.
- El **GP (*General Partner* / gestor)** es quien **gestiona** el fondo: define la estrategia, busca las
  inversiones, las ejecuta, las administra y las vende. Toma las decisiones y **responde** por ellas. El GP
  aporta normalmente una pequeña parte del capital (*GP commitment*, para "tener piel en el juego") y se
  lleva las comisiones.
- El **contrato que los une** es el ***LPA (Limited Partnership Agreement)*** —en vehículos extranjeros— o
  las reglas del vehículo mexicano (CKD/CERPI). Regula: la estrategia, los compromisos de capital, las
  comisiones, la gobernanza, los conflictos de interés, y la distribución de las ganancias.

**Las comisiones: el "2 y 20".** El modelo de negocio del GP:
- **Management fee (~2% anual):** comisión sobre el capital comprometido/gestionado, que paga los gastos
  operativos del GP (equipo, oficina). Es ingreso **fijo**.
- **Carried interest / carry (~20%):** la **participación en las ganancias** del fondo, por encima de un
  **hurdle rate** (rendimiento mínimo garantizado a los LPs, p. ej., 8%). Es el **gran incentivo**: el GP
  gana de verdad cuando el fondo genera retornos. Alinea al GP con los LPs (ambos ganan si las inversiones
  van bien). El *carry* suele estar sujeto a *vesting* y a mecanismos de **clawback** (devolver si al final
  el fondo no alcanzó el hurdle).

**El ciclo de vida del fondo (típicamente ~10 años).**
1. **Fundraising (captación):** el GP **levanta** el fondo, convenciendo a los LPs de comprometer capital.
   Define el tamaño objetivo, la estrategia y los términos (LPA).
2. **Periodo de inversión (~años 1-5):** el GP **invierte** el capital en empresas (las *portfolio
   companies*), haciendo *capital calls* (llamadas de capital a los LPs) conforme cierra cada inversión.
3. **Gestión / creación de valor (~años 3-8):** el GP **trabaja** las empresas del portafolio
   (profesionaliza, hace crecer, add-ons), buscando aumentar su valor.
4. **Desinversión / exit (~años 5-10):** el GP **vende** las empresas (trade sale, secondary, IPO) y
   **distribuye** las ganancias a los LPs (devolviendo primero su capital + hurdle, luego repartiendo el
   resto 80/20).
5. **Liquidación:** el fondo se cierra. Mientras tanto, el GP suele estar **levantando el siguiente fondo**.

**Los vehículos en México.** Para que las **afores** (la gran bolsa de capital institucional mexicano)
puedan invertir en capital privado, se crearon vehículos bursátiles: los **CKDs** (Certificados de Capital
de Desarrollo) y **CERPIs** (Certificados de Proyectos de Inversión), regulados por la LMV y la CNBV. Estos
vehículos cotizan en bolsa pero invierten en activos privados, conectando el ahorro para el retiro con el
capital privado. Muchos fondos usan además estructuras *offshore* (Delaware, Cayman) por eficiencia fiscal
y familiaridad de los inversionistas globales. El abogado de fondos estructura estos vehículos —una práctica
especializada y muy técnica—.

**Por qué importa para el abogado de la operación.** Aunque tú asesores la **inversión concreta** (la
compra de una empresa por el fondo), entender la **estructura del fondo** explica el **comportamiento** del
cliente: por qué tiene **prisa por invertir** (el periodo de inversión corre), por qué **debe vender** (el
fondo tiene fecha de cierre), por qué le obsesiona el **retorno y el exit** (de ahí su *carry*), y por qué
negocia tan duro las protecciones (responde ante sus LPs). El fondo no es un dueño cualquiera: es un
**gestor de dinero ajeno con una fecha de salida y un incentivo de retorno**. Eso explica todo lo demás.

### 14.B · El private equity y el LBO (comprar, mejorar, vender — con apalancamiento)

El **private equity** invierte en empresas **maduras**, normalmente **comprando el control**. Su modelo,
en una frase: **comprar bien, crear valor, vender mejor** —y amplificar el retorno con **deuda** (el LBO)—.

**La tesis de inversión.** Antes de comprar, el fondo define una **tesis**: por qué esta empresa, a este
precio, puede valer mucho más en 5 años. Las palancas de creación de valor:
- **Mejora operativa:** profesionalizar la gestión, optimizar costos y márgenes, mejorar la estrategia
  comercial. Crear valor **operando mejor**.
- **Crecimiento:** expansión orgánica + **buy-and-build** (adquirir empresas complementarias —*add-ons*—
  para crecer por consolidación, comprando a múltiplos bajos y vendiendo el conjunto a un múltiplo mayor).
- **Apalancamiento (el LBO):** usar **deuda** para financiar la compra, de modo que **poco equity** controle
  un activo grande, **amplificando** el retorno sobre el equity (Libro 5).
- **Multiple expansion:** vender a un **múltiplo mayor** que el de compra (comprar barato en un momento o
  sector poco valorado, vender caro cuando mejora).

**El LBO en detalle (reuniendo Libros 3 y 5).** El *leveraged buyout* es la técnica característica del PE.
Recordando y profundizando:
- **La estructura (Libro 3):** el fondo crea una **NewCo**, le aporta **equity** (su capital) y le consigue
  **deuda** (de bancos/fondos); la NewCo **compra** la *target*; luego (a menudo) **se fusiona** con ella
  (fusión apalancada), de modo que el **flujo de la target sirva la deuda**.
- **El financiamiento (Libro 5):** la deuda se estructura en **tramos** (senior + mezzanine), con un
  **intercreditor agreement**, cuidando la **asistencia financiera**, y con **garantías** sobre los activos
  y acciones de la target. El fondo asegura el ***committed financing*** para poder cerrar.
- **La matemática de la palanca:** si el fondo compra una empresa por 100 (40 de equity + 60 de deuda) y la
  vende en 5 años por 160, tras pagar la deuda (que se redujo con el flujo, digamos a 30), el equity recibe
  130 —**más de 3x** sobre los 40 invertidos—. La deuda **amplificó** el retorno. Pero si la empresa va mal
  y vale 70, tras pagar la deuda (60) el equity recibe solo 10 —**una pérdida brutal**—. La palanca
  amplifica en **ambas** direcciones; por eso el LBO es poderoso y peligroso.

**La creación de valor operativa (el PE maduro).** El PE moderno no vive solo del apalancamiento: crea
valor **mejorando la empresa**. El fondo instala gobierno corporativo profesional (consejo, métricas,
incentivos a la dirección), define un **plan de 100 días** y un **plan de creación de valor**, hace
adquisiciones complementarias, y prepara la empresa para una **venta atractiva**. El abogado acompaña todo:
la adquisición inicial, las adquisiciones *add-on*, la gobernanza (el SHA con la dirección), y —al final—
la **preparación y ejecución de la salida**.

**El gobierno de la empresa adquirida.** Cuando el PE compra el control, instala su gobierno vía un **SHA**
(Libro 4) con los **co-inversionistas** y la **dirección** (a la que suele dar equity con *vesting* e
incentivos —*management equity plan*—, para alinearla con la creación de valor y la salida). Aquí ves cómo
el PE reúne **M&A + estructura + financiamiento + SHA + gobierno corporativo** en una sola operación —la
síntesis de todo el Nivel IV—.

**El exit (la razón de ser).** El PE compra **para vender**. Las salidas típicas: **trade sale** (vender a
un comprador estratégico del sector), **secondary buyout** (vender a otro fondo de PE), o **IPO** (sacar la
empresa a bolsa). Toda la operación —desde la compra— se diseña pensando en **maximizar el valor de la
salida** en el horizonte del fondo. El abogado que entiende esto estructura **desde el inicio** pensando en
**cómo se venderá** (que la empresa sea "vendible": gobierno limpio, contratos en orden, sin contingencias
—la due diligence inversa—).
